...
const HeadCoinInfo = (props) => {
    ...
    <Box >
            ...
            <Typography variant='h6'>({props.symbol})</Typography>
             ...
             <Typography variant='h6'>${props.current_price}</Typography>  
             { props.price_change_percentage_24h <0  ? <ArrowDropDownIcon color={'warning'}/> : <ArrowDropUpIcon color={'success'}/>}
             <Typography variant='h6' color={props.price_change_percentage_24h< 0? 'red':'green'}>{props.price_change_percentage_24h.toFixed(1).toLocaleString()}</Typography>
            </Box>
    </Box> );
}
